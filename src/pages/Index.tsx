      ...
      <div className="ol-wrap">
        <div className="ol-about__in">
          ...
          <div className="ol-about__content">
            ...
            <p className="ol-about__body ol-reveal ol-d2">Создала авторский <strong>«Метод Живого Слова»</strong> — психология + стихотерапия + МАК. Куратор сообщества "ММ ВЛ".</p>
      ...
      <div className="ol-wrap">
        ...
        <div id="zhivaya-ya" className="ol-f-card ol-f-card--group ol-reveal">
          <div>
            ...
            <h3 className="ol-f-title">Терапевтическая группа<br/><em>"Живая Я"</em></h3>
...
function FAQ() {
  ...
  const faqs = [
    ...
    {q:"Что входит в сборник «Собеседница Души»?",a:"",jsx:<><p style={{marginBottom:"10px"}}>Это не просто сборник стихов.<br/>Это твой личный дневник, с которым ты работаешь, а не просто читаешь.</p><p style={{marginBottom:"10px"}}>Каждый разворот устроен как мини-сессия:</p><p style={{marginBottom:"6px"}}>📖 Авторский текст и терапевтический стих</p><p style={{marginBottom:"6px"}}>🔍 Вопросы для саморефлексии</p><p style={{marginBottom:"6px"}}>✍️ Психологическое упражнение</p><p style={{marginBottom:"10px"}}>И заключительная философская нить мини-сессии для читателя.</p><p>Как итог — рождается формат, которого раньше не было: <em>стихотерапевтический дневник</em>, как личная точка входа на пути к душе.</p></>},
        ...
        <div className="ol-faq__list">
          {faqs.map((f,i) => (
            <div key={i} className="ol-faq__item">
              ...
              <div className={`ol-faq__a${open===i?" open":""}`}>{f.jsx ?? f.a}</div>
            </div>
          ))}
        </div>
      </div>
...
function GiftWidget() {
  ...
  return (
    <>
      ...
      `}</style>
      <a href="https://disk.yandex.ru/i/GpPfanJVmP69iA" target="_blank" rel="noopener noreferrer" className={`gift-widget${visible ? " is-visible" : ""}`} aria-label="Персональный подарок" style={{textDecoration:"none"}}>
